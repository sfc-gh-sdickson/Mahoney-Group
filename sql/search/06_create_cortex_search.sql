...
SELECT
    ...
    CASE (ABS(RANDOM()) % 20)
        ...
        WHEN 3 THEN 'Settlement negotiations ongoing with ' || cl.client_name || '. Initial demand received for $' || (c.claim_amount_incurred * 1.5)::NUMBER(12,2)::VARCHAR || '. Our evaluation supports settlement at $' || c.claim_amount_incurred::VARCHAR || '. Multiple rounds of negotiation completed. Mediation scheduled for next month if agreement not reached. Attorney representing claimant. Client authorized settlement up to policy limits. Both parties motivated to resolve. Recommend settlement at $' || (c.claim_amount_incurred * 1.1)::NUMBER(12,2)::VARCHAR || ' to avoid litigation costs.'
        ...
        WHEN 7 THEN 'Liability claim filed against ' || cl.client_name || ' alleging ' || c.claim_category || '. Claimant seeking damages for injuries sustained. Hired defense counsel. Discovery process initiated. Depositions scheduled. Client concerned about potential verdict exposure. Defendant has strong defense based on comparative negligence. Expert witnesses retained for both medical and liability opinions. Settlement discussions commenced. Plaintiff demand: $' || (c.claim_amount_incurred * 2)::NUMBER(12,2)::VARCHAR || '. Defense evaluation: $' || c.claim_amount_incurred::VARCHAR || '. Recommend attempting settlement at mediation.'
        ...
        WHEN 15 THEN 'D&O claim involving ' || cl.client_name || ' board of directors. Shareholder derivative action filed. Allegations of breach of fiduciary duty. Securities counsel retained for defense. Complex corporate governance issues. Board acted on advice of counsel and financial advisors. Defense appears strong. Discovery extensive. Depositions of board members completed. Plaintiff settlement demand: $' || (c.claim_amount_incurred * 3)::NUMBER(12,2)::VARCHAR || '. Coverage under D&O policy confirmed. Defense costs significant. Recommend settlement discussions at appropriate time. Board concerned about reputation and business impact.'
