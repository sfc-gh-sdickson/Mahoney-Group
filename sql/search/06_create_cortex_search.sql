...
SELECT
    ...
    CASE (ABS(RANDOM()) % 20)
        ...
        WHEN 19 THEN 'Claim closed - full and final release obtained. ' || cl.client_name || ' claim concluded successfully. Total paid: $' || c.claim_amount_paid::VARCHAR || ' which is ' || ROUND(((c.claim_amount_paid / NULLIF(c.claim_amount_reserved, 0)) * 100), 2)::VARCHAR || '% of initial reserve. No coverage disputes. No bad faith allegations. Client satisfied with outcome. Adjuster file quality excellent. Claim handling time: ' || COALESCE(DATEDIFF('day', c.report_date, c.settlement_date), 0)::VARCHAR || ' days. Meets all KPI targets. File closed and archived. Positive outcome for all parties. Client relationship strengthened through professional claims handling.'
